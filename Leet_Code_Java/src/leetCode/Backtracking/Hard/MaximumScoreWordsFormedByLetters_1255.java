...
public class MaximumScoreWordsFormedByLetters_1255 {
    public int maxScoreWords(String[] words, char[] letters, int[] score) {
        ...
        backtrack(words, lettersCounter, score, 0, 0, totalScore);
        return totalScore[0];
    }

    private void backtrack(String[] words, Map<Character, Integer> letterCounter, int[] score, int index, int currScore,
                           int[] totalScore) {
        ...
        for (int i = index; i < words.length; i++) {
            ...
            if (isValid) {
                backtrack(words, tmpCounter, score, i + 1, currScore + wordScore, totalScore);
            }
        }
    }
}
